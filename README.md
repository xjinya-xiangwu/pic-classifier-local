...
| 状态显示「模型已就绪」但识别全部失败 | 旧版缺陷：mlx-vlm 0.7 起推理服务按模型路径精确匹配请求，旧代码发的是模型名 → 每张 500，而就绪检查只看端口通不通，掩盖了错误（已修复：请求改发完整路径 + 就绪前用小图实测一次，实测不过会如实显示失败原因） | 重新执行安装命令升级后再识别；若仍失败，点「失败 N 张 (点看原因)」截图反馈 |
